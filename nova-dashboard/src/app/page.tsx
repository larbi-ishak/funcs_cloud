...
import type { NovaFunction, FunctionsResponse } from "../types/api";

const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3002";
const GATEWAY_URL = process.env.NEXT_PUBLIC_GATEWAY_URL || "http://localhost:8081";

export default function Dashboard() {
    const { data, error, isLoading, mutate } = useSWR<FunctionsResponse>(
        `${API_URL}/functions`,
        fetcher,
    ...
    const replenishFunction = async (id: string) => {
        try {
            ...
            toast.success("Replenishment triggered");
        } catch {
            toast.error("Failed to replenish");
        }
    };
    ...
    return (
        <div className="space-y-8">
            <PageHeader
                title="Functions"
                description="Manage your deployed serverless functions and warm pools."
            />

            {error && (
                <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 text-red-500 text-sm">
                    Failed to fetch functions: {error.message}
                </div>
            )}
